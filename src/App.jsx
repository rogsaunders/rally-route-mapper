  ...
  React.useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        // Optional fallback to Sydney
        setStartGPS({ lat: -33.8688, lon: 151.2093 });
      },
  ...
  return (
    <div className={`flex flex-col h-screen font-heading ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
      ...
      <div className="flex flex-1 overflow-hidden gap-4">
        {/* LEFT: Map column */}
        <div className="w-full lg:w-1/2 h-full overflow-y-auto p-4 space-y-6 border-l border-gray-300 dark:border-gray-700">
          <MapContainer
            center={
              startGPS
                ? [startGPS.lat, startGPS.lon]
                : [-33.8688, 151.2093] // fallback in case null
            }
            zoom={14}
        ...
        {/* RIGHT: Info panel */}
        <div className="w-full lg:w-1/2 h-full overflow-y-auto p-4 space-y-6">
          <section className="space-y-2">
            <h2 className="text-lg font-semibold">📝 Route Info</h2>
            
            <input className="w-full p-2 rounded bg-gray-100 dark:bg-gray-800" placeholder="Route Name" />
