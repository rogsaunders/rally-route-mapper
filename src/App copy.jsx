    ...
    const loadSections = async () => {
      ...
      const sectionsWithData = await Promise.all(
        sectionList.map(async (section) => {
          ...
          if (wpError) {
            console.error(
              `⚠️ Failed to load waypoints for section ${section.name}`,
              wpError
            );
            return { ...section, data: [] };
          }
...
${data
  .map(
    (wp) =>
      `<wpt lat="${wp.lat}" lon="${wp.lon}"><name>${wp.name}</name><desc>${
        wp.poi || ""
      }</desc></wpt>`
  )
  .join("\n")}
        ...
        <div className="w-full lg:w-1/2 h-full overflow-y-auto p-4 space-y-4 border-t lg:border-t-0 lg:border-l border-gray-300">
          ...
          <section>
            ...
            <button
              ...
              className="bg-yellow-500 text-black px-4 py-2 rounded w-full mt-2"
            >
              ➕ Start New Section
            </button>
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      onClick={() => exportAsJSON(section.data, section.name)}
                    >
                      JSON
                    </button>
                    <button
